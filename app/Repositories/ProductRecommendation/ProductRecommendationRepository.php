...
class ProductRecommendationRepository extends BaseRepository implements ProductRecommendationRepositoryInterface
{
    ...
    public function getProductRecommen($userId)
    {
        ...
        if ($transactions != null) {
            ...
            // Lấy danh sách mã sản phẩm gợi ý dựa trên K=6 giao dịch gần nhất
            $recommendedItems = [];
            if (count($distances) >= 6) {
                for ($i = 0; $i < 6; $i++) {
                ...
                $products = Product::where('id', 'like', 0)->get();
                foreach ($recommendedItems as $pro) {
                    $data = Product::where('id', 'like', $pro)->get();
                    $products = $products->concat($data);
                }
                return $products;
            } else {
                // Xử lý khi không có đủ giao dịch để gợi ý
                return null;
            }
        } else
            return null;
        ...
        $recommendedItems = recommendItems(7, 6); // Đây là ví dụ, bạn có thể thay đổi K (số lượng sản phẩm gợi ý)
